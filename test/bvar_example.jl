...
for j in 1:size(mForecast, 3)
    plot!(pFore, (size(y, 1) - H):(size(y, 1) + H),
        [y[(size(y, 1) - H):end, j]; median(mForecast[:, :, j], dims = 1)'],
        legend = false, subplot = j);
    plot!(pFore,
        (size(y, 1) + 1):(size(y, 1) + H),
        [[percentile(mForecast[:, i, j], 16) for i in 1:H] median(mForecast[:, :, j], dims = 1)' [percentile(
                                                                                                      mForecast[:, i, j],
                                                                                                      84)
                                                                                                  for i in
                                                                                                      1:H]],
        color = "red",
        line = [:dash :solid :dash],
        subplot = j)
end
