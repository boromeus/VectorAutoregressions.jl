...
#--------------------Load Data-------------------------------------------
X,
names = readdlm(joinpath(path, "test", "gertler_karadi_var_data.csv"), ',', header = true)
...
#------------------Hyperparameters---------------------------------------
α = [0.05, 0.10, 0.15, 0.25] # confidence interval
intercept = true
K = size(Y, 2)
nrep = 1000
H = 50
...
let counter = 0
    for i in collect(3:K:15)
        counter += 1
        plot!(pIRF_asy, [mIRFa.IRF[i, :] mIRFa.CI.CIl[i, :] mIRFa.CI.CIh[i, :]],
            color = ["red" "red" "red"],
...
for i in [3 1 2 4]
    plot!(pIRF, [IRF.IRF[:, i] IRF.CI.CIl[4, :, i] IRF.CI.CIh[4, :, i]], subplot = i,
        legend = false, color = ["blue" "blue" "blue"], line = [:solid :dash :dash])
end
gui(pIRF)
